...
struct data_mem {
  ...
  int tam_so;
  int tam_pagina;
  int nframe_so;     // cantidad de frames que ocupa el so, desde el 0 hasta el
                     // (nframe_so - 1)
...
// funciones
// crea tabla de frames y hace calculos, requiere tam mem, tam so, tam pag
...
int main(int argc, char **argv) {
  const char *opc[8] = {"Tamaño memoria Real", "Tamaño SO",
                        "Tamaño Pagina",       "Ejecutar Proceso",
                        "Terminar Proceso",    "Mostrar Datos",
                        "Mapeo Logico-Fisco",  NULL};
  int opcion = 0;
  struct data_mem mem;
  ...
  do {
    ...
    switch (opcion) {
    case 1:
      printf("Ingrese (en KB) tamaño memoria real:");
      mem.tam_memoria = ingreso_int();
      break;
    case 2:
      printf("Ingrese (en KB) tamaño SO:");
      mem.tam_so = ingreso_int();
      break;
    case 3:
      printf("Ingrese (en KB) tamaño pagina:");
    ...
    case 4: // ejecutar proceso
      ...
      printf("Ingrese (en KB) tamaño proceso:");
      int tpro = ingreso_int();
      int npag_pro = tpro / mem.tam_pagina;
      if (tpro % mem.tam_pagina)
        npag_pro++;
      // valido
      if (mem.nframe_libres < npag_pro) {
        ...
        if (mem.bcp)
          mem.bcp =
              (struct bcp *)realloc(mem.bcp, sizeof(struct bcp) * mem.nproc);
        else
          mem.bcp = (struct bcp *)malloc(sizeof(struct bcp) * mem.nproc);
        ...
        for (; i < mem.nframe && canp < pp->npag_pro;
             i++, pf++) { // recorro la tabla de marcos
          if (pf->libre == 0) {
    ...
    case 7: // mapeo fisico
      break;
    case 99:
      if (!ingreso_sino("Realmente quiere salir?"))
        opcion = 0;
      break;
    }
  } while (opcion != 99);
  ...
  if (mem.bcp) {
    ...
    for (ii = 0; ii < mem.nproc; ii++, pbcp++) {
      if (pbcp->tabla_pagina)
        free(pbcp->tabla_pagina);
    }
    free(mem.bcp);
  }
...
void crear_tabla_frames(struct data_mem *m) {
  if (m->tam_memoria && m->tam_so && m->tam_pagina) {
    ...
    int i;
    for (i = 0; i < m->nframe_so; i++)
      ocupar_frame(m, i, 1);

  } else
    printf("crear_tabla_frames(): Error, faltan datos para calculo!\n");
}
...
void mostrar_data(struct data_mem *m, int opcion) {
  ...
  printf("Tam. Memoria: %d\nTam. SO: %d\nTam. Pag: %d\nN frames SO: %d\nN "
         "frames Mem: %d\nN frames libres: %d\n",
         m->tam_memoria, m->tam_so, m->tam_pagina, m->nframe_so, m->nframe,
         m->nframe_libres);
  // mostrar marcos
  if (m->fr) {
